...
from tidy.actions import Executor, collision_free
from tidy.extract import extract, normalize_filename, tokenize
from tidy.learning import feedback
from tidy.model import NaiveBayesModel
from tidy.store import Store
from tidy.types import Feature, FeatureVector
from tidy.vision import derived, postprocess
import numpy as np
...
def test_atomic_model_roundtrip(tmp_path):
    model = NaiveBayesModel(tmp_path / "model.json"); model.apply([("과제", "School", 8)]); model.save()
    assert NaiveBayesModel(tmp_path / "model.json").tokens["과제"]["School"] == 8


def test_dry_run_and_collision(tmp_path):
    source = tmp_path / "invoice.pdf"; source.write_text("invoice")
    store = Store(tmp_path / "state.db"); executor = Executor(store, dry_run=True)
    destination = executor.execute(source, tmp_path / "sorted", "Finance")
    assert source.exists() and not destination.exists()
    destination.parent.mkdir(parents=True); destination.write_text("old")
    assert collision_free(destination).name == "invoice (2).pdf"
    store.close()


def test_text_extraction(tmp_path):
    path = tmp_path / "회의록.txt"; path.write_text("meeting agenda 회의록을", encoding="utf-8")
    vector = extract(path)
    assert {f.t for f in vector.features} >= {"회의록", "meeting", "agenda", "txt"}


def test_vision_postprocess_and_derived():
    output = np.zeros((1, 84, 2), dtype=np.float32)
    output[0, :4, 0] = [320, 320, 200, 200]; output[0, 4, 0] = .9
    output[0, :4, 1] = [100, 100, 50, 50]; output[0, 4 + 63, 1] = .8
