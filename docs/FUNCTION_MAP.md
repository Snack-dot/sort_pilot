...
| `E5SubjectClassifier`, `E5SubjectClassifier.__init__`, `E5SubjectClassifier._profile_embeddings`, `E5SubjectClassifier._lexical_scores`, `E5SubjectClassifier._filename_scores`, `E5SubjectClassifier._pmi_scores`, `E5SubjectClassifier._language_scores`, `E5SubjectClassifier.classify` | `classification/e5.py` | Cache averaged profile vectors, score bounded Kiwi terms and PMI collocation phrases separately against inspectable natural profiles, resolve at most one catalog-bounded subject via longest filename-alias match, boost 영어 alone when extracted text is predominantly Latin-script, rank only catalog subjects, and retain each contribution plus top-two margin. |
...
| `_ClickOnlyComboList`, `_ClickOnlyComboList.__init__`, `_ClickOnlyComboList.mousePressEvent`, `_ClickOnlyComboList.mouseMoveEvent`, `_ClickOnlyComboList.mouseReleaseEvent`, `ClickOnlyComboBox`, `ClickOnlyComboBox.__init__`, `ClickOnlyComboBox.mousePressEvent`, `ClickOnlyComboBox.mouseMoveEvent`, `ClickOnlyComboBox.mouseReleaseEvent`, `ClickOnlyComboBox.wheelEvent` | `educational_preview.py` | Keep every preview selector click-only by rejecting drag selection, drag/drop, and wheel-based value changes. |
| `PreviewGroup`, `PreviewGroup.count`, `PreviewGroup.file_preview`, `build_preview_groups` | `educational_preview.py` | Group initial AI results by proposed destination, keep Needs Review last, and render one representative filename plus the remaining count. |
...
| `_available_destination`, `freeze_organization_plan`, `_confirmation_message` | `educational_preview.py` | Resolve existing and within-batch collisions once, return the immutable approved plan, and show grouped destinations with an exact representative in final confirmation. |
| `EducationalPreviewDialog`, `EducationalPreviewDialog.__init__`, `EducationalPreviewDialog._open_group_details`, `EducationalPreviewDialog._apply_bulk_destination`, `EducationalPreviewDialog._apply_group_destination`, `EducationalPreviewDialog._sync_bulk_destination`, `EducationalPreviewDialog._update_approve_button`, `EducationalPreviewDialog._set_read_only`, `EducationalPreviewDialog._axis_selector` | `educational_preview.py` | Build the destination-grouped summary, apply one Desktop/Downloads choice to every group with optional per-group exceptions, open fixed-choice details only on demand, and show the approved count. |
...
| `simple_classifier_enabled` | `simple_classifier.py` | Enable the explicit non-production classifier only when `SORT_PILOT_SIMPLE_CLASSIFIER` is set to a recognized true value. |
| `_simple_axis_decision`, `classify_for_ui_test` | `simple_classifier.py` | Build deterministic extension-based fixed-choice results for UI testing without loading E5/Gemma or authorizing file moves. |
