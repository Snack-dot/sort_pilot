...
- Real-data validation: 50/50 subject and 57/57 template correct against a real 100-file folder (the exact dataset behind the original report), ground truth and runner kept strictly under the git-ignored `eval/local/`. Combined real+invented recalibration reproduced identical thresholds to invented-only, confirming generalization. Full details and numbers in `evidence.md`.
