...
class Executor:
    ...
    def execute(self, source: Path, root: Path, category: str, decision_id=None) -> Path:
        ...
        source = require_sandbox_file(source, self.sandbox_root)
        root = require_in_sandbox(root, self.sandbox_root, label="destination root")
        ...
        destination = require_in_sandbox(
            collision_free(root / category / source.name),
            self.sandbox_root,
            label="destination",
        )
        if self.dry_run:
    ...
    def undo(self, journal_id: int) -> Path:
        ...
        original_source = require_in_sandbox(
            Path(row["src"]), self.sandbox_root, label="journal source"
        )
        dst = require_sandbox_file(Path(row["dst"]), self.sandbox_root)
        src = require_in_sandbox(
            collision_free(original_source), self.sandbox_root, label="undo destination"
        )
