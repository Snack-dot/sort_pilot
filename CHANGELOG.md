...
- Separately validated the fix against the real dataset behind the original report (100 real files, made available locally, never committed): filename-only, 50/50 subject and 57/57 template predictions correct; the already-recalibrated thresholds generalized to the real data without adjustment; confident auto-route rate rose from the originally reported 16.1% to 46.0% using filename evidence alone, no body text.
- Running the *full* real pipeline (real text extraction, not filename-only) against the same 100 files surfaced one more real, pre-existing defect: `Path(file_name).stem` returns NFD-decomposed Korean text on macOS/APFS, silently breaking every filename-indicator/-alias match against the NFC-composed strings in the packaged data (Windows/NTFS doesn't decompose filenames, which is why the original report never hit this). Fixed by NFC-normalizing both sides of the comparison in `template.py` and `e5.py`. After the fix: 50/50 subject, 51/57 template correct with real extracted text (the other 6 safely reached Needs Review, none wrong-but-confident), 41% of all 100 files resolved on both axes.
